...
async function loadTimeSlots(dateStr) {
  try {
    // Si no hay servicio seleccionado aún, mostrar mensaje
    if (!STATE.selectedService) {
      console.log('⚠️ Servicio no seleccionado, mostrando mensaje al usuario');
      const grid = $id('time-grid', 'hours');
      if (grid) {
        grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; color: var(--muted); padding: 2rem; background: var(--card); border-radius: 10px; border: 2px dashed var(--border);">🔔 Por favor, selecciona un servicio primero en el Paso 2</div>';
      }
      return;
    }
    ...
    console.log('✅ Horarios disponibles recibidos:', data.slots?.length || 0, 'slots');
    ...
    // Modo offline: generar horarios sin validación
...
function renderTimeSlots(dateStr) {
  ...
  // Si la API no devolvió horarios, generar offline (sin validación)
  ...
  console.log(`⏰ Horarios disponibles cargados: ${slotCount}`);
  ...
  if (serviceSelect) {
    serviceSelect.addEventListener('change', (e) => {
      ...
      if (STATE.selectedDate) {
        console.log('🔄 Recargando horarios para el nuevo servicio...');
...
function goToStep(step) {
  ...
  // Si llegamos a confirmación (step 4), generar resumen
...
function showConfirmation() {
  ...
  if (!STATE.selectedDate || !STATE.selectedTime || !service) {
    summary.innerHTML = `<div class="summary-placeholder">📋 Selecciona día, servicio y hora para ver el resumen aquí...</div>`;
    return;
  }
  ...
  void summary.offsetWidth; // Fuerza un reflow para reiniciar la animación
  ...
  if (confirmBtn) {
    ...
    // Cambiar texto del botón si hay errores de validación
    if (!isValid) {
      confirmBtn.textContent = '⏳ Completa todos los campos';
    } else if (!(consentCheckbox && consentCheckbox.checked)) {
      confirmBtn.textContent = '☑️ Acepta los términos para reservar';
    } else {
      confirmBtn.textContent = '✅ Reservar Ahora';
    }
  }
...
function hidePwaBanner(permanent = false) {
  const banner = $id('pwa-install-banner');
  const floatBtn = $id('pwa-float-btn');
  if (!banner) return;
  ...
  if (permanent) {
    ...
    // Mostrar botón flotante cuando se cierra el banner
    if (floatBtn) {
...
function setupPwaInstallUi() {
  ...
  // Mostrar banner después de 2 segundos si no se ha descartado
  setTimeout(() => {
    if (!localStorage.getItem(INSTALL_DISMISSED_KEY)) {
      showPwaBanner();
    } else if (floatBtn) {
      // Si ya cerró el banner, mostrar botón flotante
      floatBtn.hidden = false;
    }
  }, 2000);
  ...
  if (installBtn) {
    installBtn.addEventListener('click', async () => {
      ...
      if (isIOS || (isSafari && !deferredInstallPrompt)) {
        ...
        alert('📱 Para instalar en iPhone/iPad:\n\n1. Toca el botón Compartir (ícono con flecha hacia arriba)\n2. Desplázate y selecciona "Añadir a pantalla de inicio"\n3. Toca "Agregar" para confirmar\n\n¡Listo! La app aparecerá en tu pantalla de inicio.');
        return;
      }
      
      if (!deferredInstallPrompt) {
        alert('La instalación de la app está disponible solo en navegadores compatibles (Chrome, Edge).');
        hidePwaBanner(true);
        return;
      }
  ...
  // Botón flotante reabre el banner
  if (floatBtn) {
